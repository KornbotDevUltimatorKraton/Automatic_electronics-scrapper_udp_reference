                           ...
                           if len(a['href'].split('/url?q=')) == 2:
                                  ...
                                  if data_link_menufac in check_menufac[0]:
                                      try:
                                                  ...
                                                  getname = url.split(".pdf")[0].split("/")[len(url.split(".pdf")[0].split("/"))-1]
                                                  print(getname)
                                                  filename = Path(Document+keys_data+'.pdf')
                                                  r = requests.get(url)
                                                  if int(r.status_code) == 200:
                                                         print(r.status_code) 
                                                         filename.write_bytes(r.content) 
...
def Filter_collection(Pack_drawing_json): # For TI algorithm
    ...
    for p in range(0,len(Pack_drawing_json)):
      ...
      #if str(keys_data) not in list_exsist_data:
      try:
              #Making download hear 
              url = 'https://www.ti.com/lit/ds/symlink/'+keys_data+".pdf" 
              filename = Path(Document+keys_data+'.pdf')    # Setting the category name for the directory 
              r = requests.get(url)
              if int(r.status_code) == 200:
                    print(r.status_code) 
                    filename.write_bytes(r.content)                                      
...
try:
    ...
    sock1.sendto(message,(address_send,5080)) # Sending the process 
except:
    pass  